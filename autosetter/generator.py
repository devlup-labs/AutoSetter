...
def generate_all_artifacts(
    ...
    # This allows callers to pass their own pre-configured client, or
    # lets us auto-create one with default settings.
    if hf_client is None:
        hf_client = HFModelClient()

    # ── Track which artifacts have been successfully written ──
    ...
    for spec in ARTIFACTS:
        ...
        # ── ROUTING DECISION ──
        # Check if this artifact should be generated by the HuggingFace model
        # or the existing Ollama model. The routing is determined by whether
        # the artifact's name is in the HF_MODEL_ARTIFACTS set.
        if spec.name in HF_MODEL_ARTIFACTS:
            # ── Route to HuggingFace backend (fine-tuned Qwen 7B) ──
            # This generates: statement.tex, solution.cpp, brute.cpp, checker.cpp
            if progress_callback:
                progress_callback(
            ...
            # ── Route to Ollama backend (UNCHANGED existing code path) ──
            # This generates: validator.cpp, generator.cpp, solution.wa.cpp,
            # solution.tle.cpp
            if progress_callback:
                progress_callback(
        ...
        # ── Record the written path for this artifact ──
