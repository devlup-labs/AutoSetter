"""
Unit tests for autosetter.packager (bundle structure and manifest release readiness).
"""
...
from autosetter.packager import Packager
...
def test_complete_pairs_are_packaged(tmp_path: Path):
...
def test_failed_validation_marks_manifest_unready(tmp_path: Path):
    _, tests = make_dirs(tmp_path)
    (tests / "001.in").write_text("5\n")
    (tests / "001.ans").write_text("10\n")
    write_report(tests, all_passed=False, passed_tests=1, diagnosis="checker failed")

    manifest = build(tmp_path)

    assert manifest["ready_for_release"] is False
    assert manifest["validation"]["diagnosis"] == "checker failed"


def test_untrusted_checker_recorded_in_manifest(tmp_path: Path):
    _, tests = make_dirs(tmp_path)
    (tests / "001.in").write_text("5\n")
    (tests / "001.ans").write_text("10\n")
