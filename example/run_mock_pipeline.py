...
# Add project root to sys.path to import autosetter
PROJECT_ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(PROJECT_ROOT))

from autosetter.packager import Packager
from autosetter.pipeline import TestPipeline
from autosetter.sandbox import SandboxLocalClient, ensure_testlib
...
def run_mock() -> int:
    ...
    print(f"testlib.h copied from autosetter/include/ to {testlib_path}")
    ...
    print(
        "Running test pipeline (compiling artifacts, generating test inputs, "
        "validating inputs, solving, and checking outputs)..."
    )
    report = pipeline.run()
